...
For the input dataset, permeability, meaning at least one available flow path through the material, must be ensured (this can be tested using a labeling algorithm `scipy.ndimage.label`), where only face-to-face connections are considered permeable.
